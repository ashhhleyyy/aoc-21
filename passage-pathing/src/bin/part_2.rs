...
fn main() {
    ...
    find_paths(&mut paths, &graph, &current_path, start_node.clone(), false);
    println!("{}", paths.len());
}

fn find_paths(paths: &mut Vec<Vec<Node>>, graph: &HashMap<Node, Vec<Node>>, current_path: &Vec<&Node>, start_node: Node, visited_smol: bool) {
    let connections = graph.get(&start_node).unwrap();
    for node in connections {
        let visited_smol = if node.is_smol() && current_path.contains(&node) {
            if visited_smol || node.name() == "start" {
                continue;
            } else {
                true
            }
        ...
        if node.name() == "end" {
            ...
            find_paths(paths, graph, &new_path, node.clone(), visited_smol);
        }
    }
}
